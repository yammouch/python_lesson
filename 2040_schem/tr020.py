...
class MLP(Chain):
  ...
  def __init__(self):
    ...
    with self.init_scope():
      ...
      self.l2 = L.Linear(None, self.n_out)

  def forward(self, x):
    h1 = F.sigmoid(self.l1(x))
    return self.l2(F.reshape(h1, (-1, 3*10*10)))
...
# Setup a Trainer
trainer = training.Trainer(updater, (max_epoch, 'epoch'), out='tr020_result')
...
model = MLP()
serializers.load_npz('tr020_result/model_epoch-2', model)
